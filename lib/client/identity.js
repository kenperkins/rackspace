    ...
    return function(options, callback) {
        ...
        request(opts, function(err, response, body) {
            ...
            try {
                ...
                callback({
                    message: 'Unable to parse service catalog',
                    error: e
                });
            }
        });
...
var ServiceCatalog = function(options, catalog) {
    if (!options.region && !catalog.access.user['RAX-AUTH:defaultRegion']) {
        throw ('You must specify a region or configure a default region');
    }

    var self = this;

    self.region = options.region || catalog.user['RAX-AUTH:defaultRegion'];
    self.services = {};
    self.token = catalog.access.token;

    _.each(catalog.access.serviceCatalog, function(service) {
